		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0 0 0","borderRadius":"10px 10px 0 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('dingdanquxiao','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('dingdanquxiao','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('dingdanquxiao','取消统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						取消统计
					</el-button>
				</el-row>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('dingdanquxiao','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('dingdanquxiao','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			payHandler(row){
				...
				this.$storage.set('paytable','dingdanquxiao');
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "dingdanquxiao/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
