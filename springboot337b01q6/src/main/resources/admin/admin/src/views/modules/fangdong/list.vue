		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">民宿名称</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0 0 0","borderRadius":"10px 10px 0 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('fangdong','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						审核
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 30px","fontSize":"14px","color":"#000","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="touxiang" width="200" label="logo">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('fangdong','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai17" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								编辑
							</el-button>

							<el-button class="btn8" v-if="isAuth('fangdong','私聊')" type="success" @click="chatClick(scope.row)">
	...
	export default {
		...
		methods: {
			...
			chatClick(row) {
				...
				if(row.minsumingcheng){
					this.nowname = row.minsumingcheng
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "fangdong/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
