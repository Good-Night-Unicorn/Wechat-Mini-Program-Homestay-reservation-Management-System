			...
			<view :style='{"width":"100%","padding":"0 20rpx 48rpx","position":"relative","background":"#f3f5f9","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 0px)","padding":"20rpx 0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"20rpx","background":"#ffffff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.kefangmingcheng" type="text" placeholder="客房名称" ></input>
					</view>
					<button :style='{"border":"0","padding":"0px","margin":"0","color":"#fff","borderRadius":"20rpx","background":"#03b882","width":"136rpx","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' @tap="search" class="cu-btn shadow-blur round">搜索</button>
				</view>
				...
				<view :style='{"padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					<scroll-view scroll-x="true" class="category-one" :style='{"minHeight":"100vh","margin":"0px 20rpx 0 -20rpx","borderRadius":"0px 0 0 0px","background":"#fff","display":"block","width":"200rpx","height":"100%"}'>
						<view :class='categoryName === item.fangjianleixing ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.fangjianleixing)">{{item.fangjianleixing}}</view>
					</scroll-view>
					<!-- 样式1 -->
					<view class="uni-product-list" :style='{"padding":"0","margin":"0","flexWrap":"wrap","flex":"1","display":"flex","width":"calc(100% - 220rpx)","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"20rpx 10rpx","margin":"0 0 40rpx","backgroundColor":"#fff","borderColor":"#1ccb96","borderRadius":"0","flexWrap":"wrap","borderWidth":"8rpx 0 0 0","display":"flex","width":"48%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							<view class="uni-product-title" :style='{"padding":"4rpx 0px","overflow":"hidden","whiteSpace":"nowrap","color":"#03b882","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>客房名称:{{product.kefangmingcheng}}</view>
							<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"0 0px 0 0","display":"block","width":"100%","height":"240rpx","order":"-1"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.kefangtupian)" :src="product.kefangtupian.split(',')[0]"></image>
							<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"0 0px 0 0","display":"block","width":"100%","height":"240rpx","order":"-1"}' mode="aspectFill" class="uni-product-image" v-else :src="product.kefangtupian?baseUrl+product.kefangtupian.split(',')[0]:''"></image>
							<view class="uni-product-title" :style='{"padding":"4rpx 0px","overflow":"hidden","whiteSpace":"nowrap","color":"#03b882","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>民宿名称:{{product.minsumingcheng}}</view>

							<view :style='{"color":"#999","width":"100%","margin":"0 10rpx 0 0"}'>
							...
							<view :style='{"width":"100%","padding":"8rpx 8rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('minsuxinxi','修改')) || (!userid && isAuthFront('minsuxinxi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('minsuxinxi','删除')) || (!userid && isAuthFront('minsuxinxi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 8rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('minsuxinxi','修改')) || (!userid && isAuthFront('minsuxinxi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#03b882","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#03b882","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('minsuxinxi','删除')) || (!userid && isAuthFront('minsuxinxi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#f00","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#f00","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","color":"#fff","outline":"none","borderRadius":"100%","top":"144rpx","left":"20rpx","background":"#03b882","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' class="add-btn" @click="screenBoxShow=true">筛</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","right":"20rpx","outline":"none","borderRadius":"100%","top":"140rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('minsuxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","right":"20rpx","outline":"none","borderRadius":"100%","top":"140rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('minsuxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					...
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>民宿星级</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="minsuxingjiChange(item)" v-for="(item,index) in minsuxingjiOptions" :key="index" class="screenTab" :class="searchForm.minsuxingji==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"auto","padding":"0 20rpx 0 20rpx","fontSize":"24rpx","whiteSpace":"nowrap"}'>联系方式</view>
						<input :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 16rpx 0 0","borderRadius":"40rpx","background":"#f3f5f9","flex":"1","width":"75%","fontSize":"28rpx","height":"60rpx"}' placeholder="请输入联系方式" v-model="searchForm.lianxifangshi">
					</view>
...
<script>
	export default {
		...
		async onLoad(options) {
			...
			let res = {};
			//fangjianleixing筛选
			res = await this.$api.option(`fangjianleixing`,`fangjianleixing`,{});
			this.fangjianleixingOptions = res.data;
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.lianxifangshi){
			...
			async search(){
				...
				if(this.searchForm.kefangmingcheng){
					searchForm['kefangmingcheng'] = '%' + this.searchForm.kefangmingcheng + '%'
				}
				if(this.categoryName!='全部'){
					searchForm.fangjianleixing = '%' + this.categoryName + '%'
				}
				if(this.searchForm.fangjianleixing){
				...
				if(this.searchForm.lianxifangshi){
					searchForm['lianxifangshi'] = '%' + this.searchForm.lianxifangshi + '%'
				}
                let res = {};
                if(this.userid) {
